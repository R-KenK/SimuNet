...
single.example <- simu_scan(Adj.grant,total_scan.massen,scans.to.do = "all",mode = "upper")
single.example <- summary(single.example)
single.removed <- single.example
single.removed$theoretical.sum <-
  remove_central.node(single.removed$theoretical.sum,
                      "upper",compute.EV
  )

plot(single.example)
plot(single.removed)
layout.example <- igraph::layout_with_fr(igraph::graph.adjacency(single.example$theoretical.sum,weighted = TRUE))
layout.removed <- layout.example[-5,]
...
gg_color_hue(2)
plot(single.example,layout = layout.example,vertex.label = NA,vertex.size.min = 10,vertex.color = gg_color_hue(2)[1])
plot(single.removed,layout = layout.removed,vertex.label = NA,vertex.size.min = 10,vertex.color = gg_color_hue(2)[2])
