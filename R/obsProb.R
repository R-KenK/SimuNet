...
generate_obsProb <- function(Adj,
  ...
  n <- nrow(Adj)
  nodes_names <- rownames(Adj)

  obs.prob_type <-
    determine_obs.prob_type(obs.prob_fun = obs.prob_fun)

  obs.prob <- list(
    P = calculate_obs.prob(
...
calculate_obs.prob <-
  function(obs.prob_fun,
    ...
    P <- switch(
      ...
      # n*n: lazy fix to be sure that all prob are drawn, but likely drawing
      # n probabilities too many presently
...
proportional.prob <- function(P) {
  ...
  if (any(P == 1)) {
    P <-
      P - min(P) / 2 # set the maximal value to 1 - min(P)/2 and min(P) to half the previous min(P)
  }
  P
}
